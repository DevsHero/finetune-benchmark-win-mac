...
FAST_TUNE_SAMPLES = 100 # Increased for meaningful benchmarking

MODEL_NAME = "Qwen/Qwen3-0.6B"  # Qwen3-0.6B optimized for GPU
DATASET_NAME = "databricks/databricks-dolly-15k"
NUM_EPOCHS = 1
BATCH_SIZE = 4 # Optimized batch size for better GPU utilization
...
def get_ubuntu_specs():
    """Gets Ubuntu hardware and software specifications with detailed GPU info."""
    specs = {
        "os_version": "Unknown",
        "processor": "Unknown",
        "memory": "Unknown",
    ...
    try:
            ...
            with open('/etc/os-release', 'r') as f:
                for line in f:
                    if line.startswith('PRETTY_NAME'):
                        specs["os_version"] = line.split('=')[1].strip().strip('"')
        ...
        try:
            with open('/proc/cpuinfo', 'r') as f:
                cpuinfo = f.read()
                # Get model name
                model_search = re.search(r'model name\s+:\s+(.*)', cpuinfo)
                if model_search:
                    model_name = model_search.group(1).strip()
                    # Count cores
                    cores = len(re.findall(r'processor\s+:\s+', cpuinfo))
                    specs["processor"] = f"{cores}x {model_name}"
        except:
            pass
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            if 'nvidia' in gpu_name.lower() or 'NVIDIA' in gpu_name:
                try:
...
def main():
    ...
    print("Starting Ubuntu PyTorch Fine-Tuning Benchmark...")
    ...
    # Optimized with BF16 precision for stability
    model = AutoModelForCausalLM.from_pretrained(
        MODEL_NAME, 
        device_map="cuda", # Explicitly use the GPU
        torch_dtype=torch.bfloat16  # Using BF16 for better stability
    )
    ...
    training_args = TrainingArguments(
        ...
        bf16=True,   # Enable BF16 for optimization
        gradient_accumulation_steps=GRADIENT_ACCUMULATION_STEPS,
        dataloader_pin_memory=True,  # Faster data transfer
        gradient_checkpointing=True,  # Enable gradient checkpointing to save memory
        dataloader_num_workers=4,  # Use 4 workers for Linux data loading
    ...
    summary = {
        "platform": "ubuntu_pytorch",
        "model": MODEL_NAME,
        "dataset": DATASET_NAME,
        "device_specs": get_ubuntu_specs(),
